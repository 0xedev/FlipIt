...
const FlipCoin = () => {
  ...
  async function setupContract() {
    ...
    if (!isConnected || !walletProvider) {
      ...
      await open();
      throw new Error("User is disconnected, please connect your wallet");
    }
    const ethersProvider = new BrowserProvider(walletProvider as any);
    const signer = await ethersProvider.getSigner();
    const userAddress = await signer.getAddress();
  ...
  const flip = async (
    ...
    try {
      ...
      const tokenContract = new ethers.Contract(
        ...
        signer // Use the signer directly instead of contract.provider.getSigner()
      );

      const contractBalance = await tokenContract.balanceOf(ADDRESS);
      console.log("Contract Balance:", ethers.formatUnits(contractBalance, 18));
      console.log("Required Balance:", ethers.formatUnits(requiredBalance, 18));
      if (contractBalance < requiredBalance) {
        throw new Error(
          `Contract has insufficient balance: ${ethers.formatUnits(
            contractBalance,
      ...
      setState((prev) => ({ ...prev, isApproving: true }));
      const currentAllowance = await tokenContract.allowance(address!, ADDRESS);
      if (currentAllowance < tokenAmountInWei) {
        const approveTx = await tokenContract.approve(
          ADDRESS,
          tokenAmountInWei
    ...
    if (requestId) {
      intervalId = setInterval(async () => {
        try {
          ...
          setIsFlipping(false);
          console.error("Bet status check error:", error);
        }
      }, 2000);
    }
    return () => {
